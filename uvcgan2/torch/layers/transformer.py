...
class ExtendedPixelwiseViT_s2(nn.Module):
    ...
    def __init__(
        ...
        # default target style tokens
        self.target_style_tokens = nn.Parameter(torch.empty((1, n_ext, features)))
        torch.nn.init.normal_(self.target_style_tokens)
        
        # default input style tokens
        self.input_style_tokens = nn.Parameter(torch.empty((1, n_ext, features)))
        torch.nn.init.normal_(self.input_style_tokens)
    ...
    def forward(self, x):
        ...
        # t_extra_tokens : (N, n_extra, C) - target
        t_extra_tokens = self.target_style_tokens.tile(itokens.shape[0], 1, 1)
        # i_extra_tokens : (N, n_extra, C) - input
        i_extra_tokens = self.input_style_tokens.tile(itokens.shape[0], 1, 1)

        # y : (N, L, features)
        y = self.trans_input(itokens)

        # y : (N, L + n_extra, C)
        y = torch.cat([ y, t_extra_tokens, i_extra_tokens ], dim = 1)
        y = self.encoder(y)

        # ot_extra_tokens : (N, n_extra, features) - target
        ot_extra_tokens = y[:, L:L+1, :]
        # oi_extra_tokens : (N, n_extra, features) - input
        oi_extra_tokens = y[:, L+1:, :]
        ...
        return (result, ot_extra_tokens.reshape(N, -1), oi_extra_tokens.reshape(N, -1))

class ExtendedPixelwiseViTStar(nn.Module):
    ...
    def forward(self, x):
        ...
        # added x, and result to get the input and output to transformer
        return (result, o_extra_tokens.reshape(N, -1), x, result) 
