...
class ModNetBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        y_target, mod_target, mod_input = self.inner_module(y)

        # y : (N, C, H, W)
        y_target = self.deconv(y_target, r, mod_target)
        
        return (y_target, mod_target, mod_input)
...
class ModNet_s2(nn.Module):
    ...
    def forward(self, x):
        ...
        y_target, mod_target, mod_input = self.modnet(y)
        y_target = self.layer_output(y_target)
