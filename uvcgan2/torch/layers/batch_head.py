...
class BatchHeadWrapperCond(nn.Module):
    # use the same body to predict the head (real/fake)
    # or the cls (actual class)
    def __init__(self, body, head, cls, **kwargs):
        super().__init__(**kwargs)
        self._body   = body
        self._head   = head
        self._cls    = cls
    ...
    def forward(self, x, extra_bodies = None, return_body = False):
        ...
        if extra_bodies is not None:
            all_bodies = torch.cat((y_body_last, extra_bodies), dim = 0)
            y_head     = self._head(all_bodies)
            y_cls      = self._cls(all_bodies)
        else:
            y_head = self._head(y_body_last)
            y_cls  = self._cls(y_body_last)

        y_head = y_head[:y_body_last.shape[0]]
        y_cls  = y_cls[:y_body_last.shape[0]]

        if len(y_body_main) == 0:
            result_body = y_head
            result_cls  = y_cls
        else:
            result_body = y_body_main + [ y_head, ]
            result_cls  = y_body_main + [ y_cls, ]

        if return_body:
            return (result_body, result_cls, y_body_last)

        return result_body, result_cls
