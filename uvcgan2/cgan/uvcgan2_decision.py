...
class UVCGAN2_Decision(ModelBase):
    ...
    def _setup_models(self, config):
        ...
        if config.l_discriminator.softmax_out == 1:
            # add a small linear layer for classification
            models['disc_l'] = torch.nn.Sequential(
                  nn.Linear(config.l_discriminator.model_args['latent_dim'],
                            config.l_discriminator.model_args['n_classes']),
                  nn.Softmax(dim=1)
                  )
        else:
        ...
        if self.avg_momentum is not None:
            ...
            if config.l_discriminator.softmax_out == 1:
                # added average latent discriminator
                models['avg_disc_l'] = torch.nn.Sequential(
                            nn.Linear(config.l_discriminator.model_args['latent_dim'],
                                      config.l_discriminator.model_args['n_classes']),
                            nn.Softmax(dim=1)
                            )
    ...
    def _setup_losses(self, config):
        ...
        if self.models['disc_l'] is not None:
            losses += ['pred_ab_fwd', 'pred_ba_fwd',
                       'pred_ab_bwd', 'pred_ba_bwd',
                       'pred_aa', 'pred_bb',
                       'pred_cls']
    ...
    def cycle_forward_image(self, real, label, gen_fwd, gen_bkw, disc_l, disc):
        # pylint: disable=no-self-use
        
        # (N, C, H, W)
        if self.push_decision:
            fake, fwd_mod_ft = gen_fwd.module.d_forward(real, label, self.auxiliary, self.dataset)
            # testing using similarity
            test_ssim = False
            if test_ssim:
                real_arr = real.detach().cpu().numpy().squeeze()
                fake_arr = fake[0.0].detach().cpu().numpy().squeeze()
                if len(real_arr.shape) > 1:
                    ssim = structural_similarity(real_arr, fake_arr, data_range=real_arr.max() - real_arr.min(), channel_axis=0)
                else:
                    ssim = structural_similarity(real_arr, fake_arr, data_range=real_arr.max() - real_arr.min())
                fake, fwd_mod_ft = gen_fwd.module.d_forward(real, label, self.auxiliary, self.dataset, ssim=ssim)
            reco, bkw_mod_ft = gen_bkw.module.d_forward(fake[0.0], torch.abs(label-1), self.auxiliary, self.dataset)
            # after getting the mod features get the losses between them
            fwd_cls = None if disc_l is None else disc_l(fwd_mod_ft[0.0][0])
            bwk_cls = None if disc_l is None else disc_l(bkw_mod_ft[0.0][0])
        else:
            fake, fwd_mod_ft = gen_fwd(real)
            reco, _ = gen_bkw(fake)
            _, bkw_mod_ft = gen_bkw(fake.detach()) # making sure to detach for the backward features
        
            # after getting the mod features get the losses between them
            fwd_cls = None if disc_l is None else disc_l(fwd_mod_ft)
            bwk_cls = None if disc_l is None else disc_l(bkw_mod_ft)
        ...
        return (fake, reco, consist_fake, fwd_cls, bwk_cls, fwd_mod_ft, bkw_mod_ft)

    def idt_forward_image(self, real, gen, disc_l):
        # pylint: disable=no-self-use
    ...
    def forward_dispatch(self, direction):
        if direction == 'ab':
            (
                ...
                #self.latent_imgs.y_hat_ab_fwd, self.latent_imgs.y_hat_ab_bwd,  # latent images
            ) = self.cycle_forward_image(
                self.images.real_a, self.labels.real_a, self.models.gen_ab, self.models.gen_ba, 
                self.models.disc_l, self.models.disc_b
            )
            ...
            (
                ...
                #self.latent_imgs.y_hat_ba_fwd, self.latent_imgs.y_hat_ba_bwd,  # latent images
            ) = self.cycle_forward_image(
                self.images.real_b, self.labels.real_b, self.models.gen_ba, self.models.gen_ab, 
                self.models.disc_l, self.models.disc_a
            )

        elif direction == 'aa':
            # latent predictions
            (   self.images.idt_a, self.latent_preds.pred_aa_fwd, 
                self.latent_feats.feats_aa_fwd#, self.latent_imgs.y_hat_aa_fwd
            ) = self.idt_forward_image(
                self.images.real_a, self.models.gen_ba, self.models.disc_l
            )

        elif direction == 'bb':
            # latent predictions
            (   self.images.idt_b, self.latent_preds.pred_bb_fwd, 
                self.latent_feats.feats_bb_fwd#, self.latent_imgs.y_hat_bb_fwd,
            ) = self.idt_forward_image(
                self.images.real_b, self.models.gen_ab, self.models.disc_l
            )
            ...
            (
                ...
                self.latent_preds.pred_ab_fwd, self.latent_preds.pred_ab_bwd,  # latent predictions
                self.latent_feats.feat_ab_fwd, self.latent_feats.feat_ab_bwd,   # latent features
                #self.latent_imgs.y_hat_ab_fwd, self.latent_imgs.y_hat_ab_bwd,
            ) = self.cycle_forward_image(
                self.images.real_a, self.labels.real_a,
                self.models.avg_gen_ab, self.models.avg_gen_ba, 
                self.models.avg_disc_l, self.models.disc_b
            )
            ...
            (
                ...
                self.latent_preds.pred_ba_fwd, self.latent_preds.pred_ba_bwd,  # latent predictions
                self.latent_feats.feat_ba_fwd, self.latent_feats.feat_ba_bwd,   # latent features
                #self.latent_imgs.y_hat_ba_fwd, self.latent_imgs.y_hat_ba_bwd,
            ) = self.cycle_forward_image(
                self.images.real_b, self.labels.real_b,
                self.models.avg_gen_ba, self.models.avg_gen_ab, 
                self.models.avg_disc_l, self.models.disc_a
            )
    ...
    def eval_loss_of_cycle_forward(
        ...
        loss_class_fwd = lambda_l_fwd * self.criterion_latent(pred_ab_fwd, label_a) # added latent criteria
        loss_class_bwd = lambda_l_bwd * self.criterion_latent(pred_ab_bwd, label_b) # added latent criteria - not sure if needed
        
        if self.eval_cycle_mode == 'vanilla':
            loss_class = (loss_class_fwd + loss_class_bwd) * 0.5
            loss = loss_gen + loss_cycle
        elif self.eval_cycle_mode == 'decision_fwd':
            loss_class = loss_class_fwd
            loss = loss_gen + loss_cycle + loss_class
        elif self.eval_cycle_mode == 'decision_full':
            loss_class = (loss_class_fwd + loss_class_bwd) * 0.5
            loss = loss_gen + loss_cycle + loss_class
    ...
    # backward_discriminator_base - for latent
    def backward_l_discriminator_base(
        ...
        loss_ab_fwd = self.criterion_latent(pred_ab_fwd, real_a)
        
        pred_ba_fwd = model.forward(feat_ba_fwd)
        loss_ba_fwd = self.criterion_latent(pred_ba_fwd, real_b)
    ...
    def backward_discriminators(self):
        ...
        feat_ab_fwd = self.latent_feats.feat_ab_fwd.detach()
        feat_ba_fwd = self.latent_feats.feat_ba_fwd.detach()
        
        self.losses.disc_l = \
            self.backward_l_discriminator_base(
                self.models.disc_l,
                self.labels.real_a, feat_ab_fwd, 
                self.labels.real_b, feat_ba_fwd,
            )
