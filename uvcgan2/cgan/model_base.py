...
class ModelBase:
    ...
    def __init__(self, savedir, config, is_train, device):
        ...
        if 'decision' in config.model:
            self.labels       = self._setup_labels(config) # new for labeled training
            self.latent_preds = self._setup_latent_preds(config) # new for labeled training
            self.latent_feats = self._setup_latent_feats(config) # new for labeled training
            self.latent_imgs = self._setup_latent_imgs(config) # new for labeled training
    ...
    def set_input(self, inputs, domain = None):
        ...
        # make it so that it clears everything else too
        # clear features, predictions, labels, etc.
        for key in self.latent_feats:
            self.latent_feats[key] = None
        for key in self.latent_preds:
            self.latent_preds[key] = None
        for key in self.labels:
            self.labels[key] = None
        for key in self.latent_imgs:
            self.latent_imgs[key] = None
    ...
    # added labels and latent vectors
    def _setup_labels(self, config):
        raise NotImplementedError
        
    def latent_feats(self, config):
        raise NotImplementedError
    
    def latent_imgs(self, config):
        raise NotImplementedError

    def _setup_latent_preds(self, config):
        raise NotImplementedError
    ...
    # decision forward
    def d_forward(self, dataset='breast'):
