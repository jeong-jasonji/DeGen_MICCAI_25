...
def select_base_discriminator(model, **kwargs):
    ...
    if model == 'basic_cond':
        return NLayerDiscriminatorConditioned(n_layers = 3, **kwargs)
    ...
    if model == 'pixel_conditioned':
        return PixelDiscriminatorConditioned(**kwargs)
...
class PixelDiscriminatorConditioned(nn.Module):
    """Defines a 1x1 PatchGAN discriminator (pixelGAN)"""

    def __init__(self, image_shape, ndf=64, norm='batch', n_l=2):
        """Construct a 1x1 PatchGAN discriminator
        ...
        Parameters:
            ...
            ndf (int)       -- the number of filters in the last conv layer
            norm_layer      -- normalization layer
            n_l (int)       -- number of output classes
        """
        super(PixelDiscriminatorConditioned, self).__init__()
        ...
        self.gan = nn.Conv2d(ndf * 2, 1, kernel_size=1, stride=1, padding=0, bias=use_bias)
        self.label = nn.Conv2d(ndf * 2, n_l, kernel_size=1, stride=1, padding=0, bias=use_bias)

    def forward(self, input):
        """Standard forward."""
        net_out = self.net(input)
        return self.gan(net_out), self.label(net_out)